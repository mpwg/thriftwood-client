...
class NewznabAPI {
  ...
  factory NewznabAPI.fromIndexer(LunaIndexer indexer) {
    Dio _dio = Dio(
      BaseOptions(
        ...
        queryParameters: {if (indexer.apiKey != '') 'apikey': indexer.apiKey},
  ...
  Future<List<NewznabCategoryData>> getCategories() async {
    Response response = await _dio.get('', queryParameters: {'t': 'caps'});
    ...
    xml
        ...
        .forEach((cat) {
          ...
          if ((catName?.isNotEmpty ?? false) && (catId != null)) {
            NewznabCategoryData data = NewznabCategoryData(
              id: catId,
              name: catName,
            );
            cat.findElements('subcat').forEach((subcat) {
              ...
              if ((subcatName?.isNotEmpty ?? false) && (subcatId != null))
                data.subcategories.add(
                  NewznabSubcategoryData(id: subcatId, name: subcatName),
                );
            });
            results.add(data);
          }
  ...
  Future<List<NewznabResultData>> getResults({
    ...
    XmlDocument xml = XmlDocument.parse(response.data);
    xml.getElement('rss')?.getElement('channel')?.findElements('item').forEach((
      item,
    ) {
